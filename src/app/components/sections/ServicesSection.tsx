...
function ServicePanel({
  ...
  return (
    <div
      onClick={onClick}
      className="flex flex-col h-[400px] items-start justify-between p-6 md:p-8 flex-1 min-w-0 bg-[#f9f9f9] hover:bg-[#f4f4f4] rounded-2xl hover:rounded-none transition-all duration-300 ease-out group cursor-pointer"
    >
...
export function ServicesSection() {
  return (
    <section id="services" className="bg-white w-full px-5 md:px-16 lg:px-[200px] py-16 md:py-24 lg:pt-[75px] flex flex-col gap-16">
      {/* Header row */}
      ...
      <div className="flex flex-col md:flex-row gap-2">
        <ServicePanel
          ...
          onClick={() => navigateToService("product-design")}
        />
        <ServicePanel
          title="Website Design & Development"
          desc="For businesses that need a clear, thoughtful website from design through development."
          linkLabel="EXPLORE WEBSITE"
          onClick={() => navigateToService("website")}
        />
        <ServicePanel
          title="AI Video Production"
          desc="For products and brands that need stronger visual storytelling through motion and AI-led production."
          linkLabel="EXPLORE AI VIDEO"
          onClick={() => navigateToService("ai-video")}
        />
      </div>
    </section>
  );
